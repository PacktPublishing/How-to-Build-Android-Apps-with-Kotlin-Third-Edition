...
fun DetailsView(
    ...
    DisposableEffect(key1 = lifecycleOwner) {
        val observer = LifecycleEventObserver { _, event ->
            if (event == Lifecycle.Event.ON_PAUSE) {
                ...
                Log.d("DetailsView", "DetailsView resumed")
            }
        }
    ...
    Column(
        modifier = modifier
            .fillMaxWidth()
            .padding(LocalMargin.current.margin)
    ) {
        Row {
            AsyncImage(
                model = image,
            ...
            Column(modifier = Modifier.padding(start = LocalMargin.current.margin)) {
                Text(
                    text = title,
        ...
        CompositionLocalProvider(LocalMargin provides Margin(margin = 8.dp)) {
            Text(
                text = overview,
                overflow = TextOverflow.Ellipsis,
                modifier = Modifier
                    .padding(top = LocalMargin.current.margin)
                    .fillMaxWidth()
            )
        }
    }
